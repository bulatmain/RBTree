...
namespace cust {
    ...
    protected:
        ...
        static bool rootCase(node_ptr node);
    ...
    void RBTree<T>::add(T const& value) {
        if (empty()) {
            this->root = makeNode(BLACK, value);
        } else {
            auto node = addToLeafOfSubtree(root, value);
            balanceFrom(node);
        }
        ++_size;
    }
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::findLeafParentInSubtree(node_ptr root, T const& value) {
        if (!root) {
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::addToLeafOfSubtree(node_ptr root, T const& value) {
        try {
    ...
    template <class T>
    void RBTree<T>::balanceFrom(node_ptr node) {
        // printTree(root);
        int count = 0;
    ...
    template <class T>
    bool RBTree<T>::redNode(node_ptr node) {
        return (node) && (node->color == RED);
    }

    template <class T>
    bool RBTree<T>::redParent(node_ptr node) {
        auto parent = node->parent.lock();
        return (parent) && (parent->color == Color::RED);
    }

    template <class T>
    bool RBTree<T>::noGrandFatherCase(node_ptr node) {
        auto parent = node->parent.lock();
    ...
    template <class T>
    bool RBTree<T>::leftChildParentCase(node_ptr node) {
        auto parent = node->parent.lock();
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::leftChildParentCaseBalance(node_ptr node) {
        if (rightChildNodeCase(node)) {
            node = node->parent.lock();
            node = leftRotate(node)->left;
        }
        auto grandfather = recolorParentAndGrandfather(node);
        return rightRotate(grandfather);
    }

    template <class T>
    RBTree<T>::node_ptr RBTree<T>::rightChildParentCaseBalance(node_ptr node) {
        if (leftChildNodeCase(node)) {
            node = node->parent.lock();
            node = rightRotate(node)->right;
        }
        auto grandfather = recolorParentAndGrandfather(node);
        return leftRotate(grandfather);
    }

    template <class T>
    bool RBTree<T>::redUncleCase(node_ptr node) {
        auto parent = node->parent.lock();
        if (!parent) return false;
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::recolorParentAndGrandfather(node_ptr node) {
        auto parent = node->parent.lock();
        auto grandfather = parent->parent.lock();
        parent->color = BLACK;
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::recolorParentAndUncleAndGrandfather(node_ptr node) {
        auto parent = node->parent.lock();
        auto grandfather = parent->parent.lock();
    ...
    template <class T>
    bool RBTree<T>::rightChildNodeCase(node_ptr node) {
        auto parent = node->parent.lock();
        return (parent) && (parent->right == node);
    }

    template <class T>
    bool RBTree<T>::leftChildNodeCase(node_ptr node) {
        auto parent = node->parent.lock();
        return (parent) && (parent->left == node);
    }

    template <class T>
    void RBTree<T>::restoreRootProperty() {
        root->color = BLACK;
    }

    template <class T>
    RBTree<T>::node_ptr RBTree<T>::addNodeToLeaf(node_ptr node, T const& value) {
        node_ptr leaf;
        if (*node->value == value) {
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::addNodeToLeftLeaf(node_ptr node, T const& value) {
        if (!node->left) {
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::addNodeToRightLeaf(node_ptr node, T const& value) {
        if (!node->right) {
    ...
    template <class T>
    RBTree<T>::node_ptr RBTree<T>::makeNode(Color color, T const& value) {
